...
// guest pulls from 10.0.2.2:5000 — both URLs hit the same registry.
...
target "debian-cloud" {
  ...
  cmdline = "console=${local.console},115200n8 root=LABEL=cloudimg-rootfs ro"
  disk {
...
target "from-disk" {
  ...
  cmdline = "console=${local.console} ro root=/dev/vda2"
  disk {
    device = "/dev/vda2"
    fs     = "ext4"
  }
